...
- has modules for Guava, Joda Time, Relational DB and Neo4J
...
- Defining a good API is hard, API is maybe bloated but it's kinda by design
...
#### Best exception testing: catchThrowable
...
@Test
boolean myTest() {
  assertTrue(strictlyGreaterThanTen(11));
  assertFalse(strictlyGreaterThanTen(9));
}
```
...
- chaos testing
  - introduce random fault in system
  - verify the resilience of the system
- A/B testing & tests in production
  - test the real stuff, preprod is prod: different network, config, amound of data,...
  - rely on solid metrics, dashboard and alerts
  - require to easily be able to rollback to a previous version
